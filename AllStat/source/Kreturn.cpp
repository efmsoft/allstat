...
const char* Kreturn2StrC(uint32_t value)
{
  std::string str = Kreturn2Str(value);
  return strdup(str.c_str());
}

ItemArray AllStat::KreturnInfo(uint32_t lr)
{
  TABLES t;
  KRETURNGetTables(t);
  ...
  for (auto it = a.begin(); it != a.end(); ++it)
  {
    ...
    AS_ITEM ai;
    ItemFromStatusItem(*item, ai, AS_GENERATOR::AS_KRETURN, AS_GENERATOR::AS_KRETURN);
...
uint32_t Name2KreturnItem(const char* constant_name, PAS_ITEM pitem)
{
  if (pitem == nullptr)
  {
    assert(pitem);
  ...
  ItemFromStatusItem(*e, *pitem, AS_GENERATOR::AS_KRETURN, AS_GENERATOR::AS_KRETURN);
  return 0;
}
